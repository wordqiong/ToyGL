...
float getCaustics() {
    // 波纹1
    float speed1 = float(time) / (128 * 15);
    vec2 coord1 = uv_frag/ 128;
    coord1.x *= 4;
    coord1.x += speed1*2 + coord1.y;
    coord1.y -= speed1;
    float noise1 = texture(tex_noise, coord1).x;
    noise1 = noise1*2 - 1.0;

    // 波纹2
    float speed2 =  float(time) / (128 * 15);
    vec2 coord2 = uv_frag/ 128;
    coord2.y *= 4;
    coord2.y += speed2*2 + coord2.x;
    coord2.x -= speed2;
    float noise2 = texture(tex_noise, coord2).x;
    noise2 = noise2*2 - 1.0;
